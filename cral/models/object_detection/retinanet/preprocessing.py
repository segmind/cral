...
def make_shapes_callback(model):
    ...
    def get_shapes(image_shape, pyramid_levels):
        ...
        image_shapes = [
            shape['P{}'.format(level)][1:3] for level in pyramid_levels
        ]
        return image_shapes
...
def shift(shape, stride, anchors):
    ...
    K = shifts.shape[0]
    all_anchors = (
        anchors.reshape((1, A, 4)) + shifts.reshape((1, K, 4)).transpose(
            (1, 0, 2)))
...
def anchor_targets_bbox(anchors,
    ...
    regression = np.zeros((anchors.shape[0], 4 + 1),
                          dtype=keras.backend.floatx())
    labels = np.zeros((anchors.shape[0], num_classes + 1),
                      dtype=keras.backend.floatx())

    # compute labels and regression targets
    if bboxes.shape[0]:
        assert bboxes.shape[
            1] == 4, f'Expected bboxes of dimensions (N,4), got \
            {bboxes.shape} instead'

        assert bboxes.shape[0] == gt_labels.shape[
            0], f'Expected number of annotations({bboxes.shape})\
            and labels({labels.shape}) to be same'

        # obtain indices of gt annotations with the greatest overlap
        positive_indices, ignore_indices, argmax_overlaps_inds = compute_gt_annotations(  # noqa: E501
        ...
        labels[
            positive_indices,
            gt_labels[argmax_overlaps_inds[positive_indices]].astype(int)] = 1

        regression[:, :-1] = bbox_transform(anchors,
