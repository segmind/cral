...
def filter_detections(boxes,
    ...
    Args
        ...
        other: List of tensors of shape (num_boxes, ...) to filter along with
            the boxes and classification scores.
        class_specific_filter: Whether to perform filtering per class, or take
    ...
    Returns
        ...
        of the non-suppressed boxes. Scores is shaped (max_detections,) and
        contains the scores of the predicted class. Labels is shaped
        (max_detections,) and contains the predicted label. Other[i] is shaped
        (max_detections, ...) and contains the filtered other[i] data. In case
        there are less than max_detections detections, the tensors are padded
        with -1's.
    """
    ...
    scores, top_indices = tf.nn.top_k(
        scores,
        k=keras.backend.minimum(max_detections,
    ...
    other_ = [
        tf.pad(
            o, [[0, pad_size]] + [[0, 0] for _ in range(1, len(o.shape))],
...
class FilterDetections(keras.layers.Layer):
    ...
    def __init__(self,
        ...
        Args
            ...
            class_specific_filter : Whether to perform filtering per class, or
                                take the best scoring class and filter those.
            nms_threshold: Threshold for the IoU value to determine when a box
                            should be suppressed.
            score_threshold: Threshold used to prefilter the boxes with.
            max_detections: Maximum number of detections to keep.
    ...
    def call(self, inputs, **kwargs):
        ...
        Args
            inputs : List of [boxes, classification, other[0], other[1], ...]
                    tensors.
        """
        boxes = inputs[0]
        ...
        outputs = tf.map_fn(
            ...
            dtype=[keras.backend.floatx(),
                   keras.backend.floatx(), 'int32'] + [o.dtype for o in other],
    ...
    def get_config(self):
        ...
        Returns: Dictionary containing the parameters of this layer.
        """
...
def bbox_transform_inv(boxes, deltas, mean=None, std=None):
    ...
    previously applied (in the generator) has to be removed. The mean and std
    are the mean and std as applied in the generator. They are unnormalized in
    this function and then applied to the boxes.

    Args
        boxes: np.array of shape (B, N, 4), where B is the batch size, N the
...
def shift(shape, stride, anchors):
    ...
    """
    shift_x = (
        keras.backend.arange(0, shape[1], dtype=keras.backend.floatx()) +
        keras.backend.constant(0.5, dtype=keras.backend.floatx())) * stride
    shift_y = (
        keras.backend.arange(0, shape[0], dtype=keras.backend.floatx()) +
        keras.backend.constant(0.5, dtype=keras.backend.floatx())) * stride
    ...
    shifted_anchors = keras.backend.reshape(
        anchors, [1, number_of_anchors, 4]) + keras.backend.cast(
...
class RegressBoxes(keras.layers.Layer):
    ...
    def __init__(self, mean=None, std=None, *args, **kwargs):
        ...
        Args
            mean: The mean value of the regression values which was used for
                normalization.
            std: The standard value of the regression values which was used
                for normalization.
        """
        if mean is None:
        ...
        if isinstance(mean, (list, tuple)):
            ...
            raise ValueError(
                f'Expected mean to be a np.ndarray, list or tuple. Received: \
                {type(mean)}')
        ...
        if isinstance(std, (list, tuple)):
            ...
            raise ValueError(
                f'Expected std to be a np.ndarray, list or tuple. Received: \
                {type(std)}')
...
class Anchors(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        anchors = keras.backend.tile(
            keras.backend.expand_dims(anchors, axis=0),
