...
from .mrcnn_utils import (batch_slice, box_refinement, box_refinement_graph,
                          resize_image, resize, resize_mask, minimize_mask,
                          compute_overlaps, compute_iou, extract_bboxes)
...
class ProposalLayer(KL.Layer):
    ...
    def call(self, inputs):
        ...
        pre_nms_limit = tf.minimum(self.config.PRE_NMS_LIMIT, tf.shape(input=anchors)[1])  # noqa: E501
        ...
        def nms(boxes, scores):
            ...
            proposals = tf.pad(tensor=proposals, paddings=[(0, padding), (0, 0)])  # noqa: E501
            return proposals
...
class PyramidROIAlign(KL.Layer):
    ...
    def call(self, inputs):
        ...
        shape = tf.concat([tf.shape(input=boxes)[:2], tf.shape(input=pooled)[1:]], axis=0)  # noqa: E501
...
def overlaps_graph(boxes1, boxes2):
    ...
    overlaps = tf.reshape(iou, [tf.shape(input=boxes1)[0], tf.shape(input=boxes2)[0]])  # noqa: E501
    return overlaps
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes,
    ...
    negative_indices = tf.compat.v1.where(tf.logical_and(roi_iou_max < 0.5,
                                          no_crowd_bool))[:, 0]
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    Inputs:
        ...
        deltas: [N, num_classes, (dy, dx, log(dh), log(dw))]. Class-specific
                bounding box deltas.
        window: (y1, x1, y2, x2) in normalized coordinates. The part
                of the image that contains the image excluding the padding.

    Returns detections shaped: [num_detections, (y1, x1, y2, x2, class_id,
                                score)] where coordinates are normalized.
    """
    # Class IDs per ROI
...
class DetectionLayer(KL.Layer):
    ...
    def call(self, inputs):
        ...
        # Get windows of images in normalized coordinates. Windows
        # are the area
        # in the image that excludes the padding.
        # Use the shape of the first image in the batch to normalize
        # the window
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    Returns:
        ...
        bbox_deltas: [batch, num_rois, NUM_CLASSES,
                     (dy, dx, log(dh), log(dw))] Deltas to apply to
                     proposal boxes
    """
    # ROI Pooling
...
def build_fpn_mask_graph(rois, feature_maps, image_meta,
                         pool_size, num_classes, train_bn=True):
    """Builds the computation graph of the mask head of
    ...
    Returns: Masks [batch, num_rois, MASK_POOL_SIZE, MASK_POOL_SIZE,
                    NUM_CLASSES]
    """
    # ROI Pooling
    # Shape: [batch, num_rois, MASK_POOL_SIZE, MASK_POOL_SIZE, channels]
    x = PyramidROIAlign([pool_size, pool_size],
                        name="roi_align_mask")([rois, image_meta] + feature_maps)  # noqa: E501
...
def load_image_gt(dataset, config, image_id, augmentation=None):
    ...
    if augmentation:
        ...
        assert image.shape == image_shape, "Augmentation shouldn't change image size"  # noqa: E501
        assert mask.shape == mask_shape, "Augmentation shouldn't change mask size"  # noqa: E501
    ...
    source_class_ids = dataset.source_class_ids[dataset.image_info[image_id]["source"]]  # noqa: E501
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes,
    ...
    masks: [TRAIN_ROIS_PER_IMAGE, height, width, NUM_CLASSES).
           Class specific masks cropped to bbox boundaries and resized
           to neural network output size.
    """
    ...
    masks = np.zeros((config.TRAIN_ROIS_PER_IMAGE, config.MASK_SHAPE[0], config.MASK_SHAPE[1], config.NUM_CLASSES),  # noqa: E501
                     dtype=np.float32)
    for i in pos_ids:
