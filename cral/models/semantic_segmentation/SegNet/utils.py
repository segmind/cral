...
class SegNetConfig(object):
    """docstring for Deeplabv3Config."""

    def __init__(self, height=576, width=576, num_upsample_layers=3):
        self.height = height
        self.width = width
...
def log_SegNet_config_params(config):

    assert isinstance(config,
                      SegNetConfig), 'config not supported {}'.format(config)
    # config_data = vars(config)
    # log_params(config_data)


class SegNetPredictor(object):
