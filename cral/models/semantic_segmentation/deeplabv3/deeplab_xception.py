"""Deeplabv3+ model for Keras. This model is based on TF repo:
https://github.com/tensorflow/models/tree/master/research/deeplab On Pascal
VOC, original model gets to 84.56% mIOU MobileNetv2 backbone is based on this
repo: https://github.com/JonathanCMitchell/mobilenet_v2_keras.
...
- [Inverted Residuals and Linear Bottlenecks: Mobile Networks for
    Classification, Detection and Segmentation](https://arxiv.org
    /abs/1801.04381)
"""
...
WEIGHTS_PATH_X_CS = 'https://github.com/bonlime/keras-deeplab-v3-plus/releases/download/1.2/deeplabv3_xception_tf_dim_ordering_tf_kernels_cityscapes.h5'  # noqa: E501
WEIGHTS_PATH_MOBILE_CS = 'https://github.com/bonlime/keras-deeplab-v3-plus/releases/download/1.2/deeplabv3_mobilenetv2_tf_dim_ordering_tf_kernels_cityscapes.h5'  # noqa: E501
    ...
    """ Basic building block of modified Xception network
        Args:
            ...
            depth_activation: flag to use activation between depthwise &
                            pointwise convs
            return_skip: flag to return additional tensor after 2 SepConvs
                        for decoder
            """
    residual = inputs
    for i in range(3):
...
def Deeplabv3(weights=None,
              ...
              alpha=1.,
              activation=None):
    """Instantiates the Deeplabv3+ architecture Optionally loads weights pre-
    trained on PASCAL VOC or Cityscapes. This model is available for TensorFlow
    only.
    ...
    Arguments
        ...
        input_shape: shape of input image. format HxWxC PASCAL VOC model was
                trained on (512,512,3) images. None is allowed as shape/width
        ...
        OS: determines input_shape/feature_extractor_output ratio. One of
            {8,16}. Used only for xception backbone.
        alpha: controls the width of the MobileNetV2 network. This is known
            as the width multiplier in the MobileNetV2 paper.
            ...
            Used only for mobilenetv2 backbone. Pretrained is only available
            for alpha=1.
    Returns
    ...
    x = Conv2D(
        256, (1, 1), padding='same', use_bias=False,
        name='concat_projection')(x)  # noqa: E501
    ...
    if weights == 'pascal_voc':
        ...
        if backbone == 'xception':
            ...
            weights_path = get_file(
                'deeplabv3_mobilenetv2_tf_dim_ordering_tf_kernels_cityscapes.h5',  # noqa: E501
...
def DeepLabV3Plus_xception(config, num_classes, weights, base_trainable):
    ...
    if weights not in (None, 'pascal_voc', 'cityscapes'):
        print(
            f'weights `{weights}` is not supported .. setting it to `{weights}`'  # noqa: E501
        )
        weights = 'pascal_voc'
